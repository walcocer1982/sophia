...
export default function EngineChatLayout({
  ...
  const mediaImages: string[] = useMemo(() => {
    try {
      ...
      const mediaRoot = (vm as any)?.media || {};
      const entry = mediaRoot?.[stepCode || ''] || {};
      let images: any = entry?.images;
      // Fallback por momento (p.ej., usar "M1" cuando no hay exacto para "M1-EL01")
      if ((!Array.isArray(images) || images.length === 0) && stepCode) {
        const momentKey = String(stepCode).split('-')[0];
        const entryMoment = mediaRoot?.[momentKey] || {};
        images = entryMoment?.images;
      }
  ...
  const currentMultimedia: MediaItem[] = useMemo(() => {
    try {
      ...
      if ((!items || items.length === 0) && stepCode) {
        const momentKey = String(stepCode).split('-')[0];
        const entryMoment = mediaRoot?.[momentKey] || {};
        items = Array.isArray(entryMoment?.items) ? entryMoment.items : [];
      }
