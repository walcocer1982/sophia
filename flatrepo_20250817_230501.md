...
repository:
  ...
  url: ""
generated:
  timestamp: 2025-08-18T04:05:02.051Z
  tool: FlatRepo
statistics:
  totalFiles: 49
  totalLines: 4603
...
export const TimelineStepSchema = z.object({
	...
	// NUEVO: expectativas que orientan evaluación y pistas
	expected: z.array(z.string()).optional(),
	// NUEVO: banco opcional de pistas predefinidas por step
	hints: z.array(z.string()).optional(),
	// NUEVO: tipo de respuesta (abre camino a rúbrica "open")
	answer_type: z.enum(['open','closed']).optional(),
...
import { pickModelWithBudget, getBudgetManager } from './costs';
...
export function isStudentAskingQuestion(input: string): boolean {
  ...
  const PHRASES = [
    ...
    'tengo una duda','tengo una consulta','una duda','una consulta','puedo consultar',
    'te puedo preguntar','quiero preguntar','no entiendo','no comprendo','puedes aclarar',
    'puedes explicar','qué significa','como es que','por qué','cuál es','dónde','cuándo','quién','qué es'
  ];
...
export function isAffirmativeToResume(input: string): boolean {
  const n = (input||'').toLowerCase().trim();
  return ['sí','si','ok','listo','entendido','claro','ya'].includes(n) || /^si([ ,.!?]|$)/i.test(n);
}
```
...
export function extractKeywords(from: (string|undefined)[]): string[] {
	const text = (from || []).filter(Boolean).join(' ').toLowerCase();
	const tokens = text
		.normalize('NFD').replace(/[\u0300-\u036f]/g,'')
		.replace(/[^a-z0-9áéíóúñ\s]/gi,' ')
		.split(/\s+/).filter(Boolean);
	const stop = new Set(['de','la','el','los','las','y','o','en','un','una','para','que','del','al','con','por','su','sus','se','es','son']);
	const uniq: string[] = [];
...
export function makeObjectiveHint(args: {
	...
	const cues = (missing?.length ? missing : expected).slice(0,2);
	const keywords = extractKeywords([objective, ...expected]).slice(0,6);
	const variant = variants[0] || ''; // rota con hintsUsed si quieres
	const core = keywords.length ? `Enfócate en ${keywords.join(', ')}.` : `Enfócate en el objetivo.`;
	const cueLine = cues.length ? `Menciona: ${cues.join(', ')}.` : '';
	const msg = [variant, core, cueLine].filter(Boolean).join(' ').trim();
	return msg.length > wordLimit*6 ? msg.slice(0, wordLimit*6) : msg;
}
...
export function makeObjectiveReask(args: {
	...
	switch (answerType) {
		case 'definition': return `Define en ${maxWords} palabras ${base}.`;
		case 'procedure':  return `Enumera en ${maxWords} palabras 2 pasos clave de ${base}.`;
		case 'choice':     return `Indica en ${maxWords} palabras la opción que aplica sobre ${base}.`;
		default:           return `Menciona en ${maxWords} palabras 2 elementos de ${base}.`;
	}
}
...
export function makeOpenHint({ objective, aspects = [], minWords = 12 }: {
	...
	return [
		...
		cues ? `Guíate por: ${cues}.` : '',
		`Conecta con tu trabajo o experiencia.`
	].filter(Boolean).join(' ');
}
...
export function makeOpenReask({ objective, aspects = [], minWords = 12 }: {
	...
	const a = (aspects||[]).slice(0,2).join(' y ') || 'tus ideas principales';
...
export async function POST(req: Request) {
    ...
    let skipGuarantee = false;
        ...
        if (act.kind === 'ask') {
				...
				const attemptsSoFar = Number(state.attemptsByAskCode?.[stepCode] || 0);
				const lastActionForStep = String(state.lastActionByAskCode?.[stepCode] || 'ask');
				...
				// Usar expected del JSON si está disponible, sino derivar desde pasos previos
				let expected: string[] = Array.isArray(act.step.data?.expected) ? act.step.data.expected : [];
				if (!expected.length) {
					try {
          ...
          // --------- GATE EN SALUDO/CONEXIÓN (no aceptar por débil evidencia) ---
          const momentCode = String(state.plan?.moments?.[act.step.momentIndex]?.code || '');
          if (['SALUDO','CONEXION'].includes(momentCode) && cls?.kind === 'ACCEPT' && answerType !== 'open') {
            const matchedCount = (cls.matched || []).length;
            if (matchedCount < 2) cls.kind = 'HINT' as any;
          }
          ...
          if (!vague && cls.kind === 'ACCEPT') {
            ...
            // SOLO aquí avanzas el puntero
            state.answeredAskCodes = Array.from(new Set([...(state.answeredAskCodes||[]), String(act.step.data?.code || '')]));
            state = next(state);
            state.justAskedFollowUp = false;
            state.lastFollowUpText = '';
            SESSIONS.set(sessionKey, state);
            try { await getSessionStore().set(sessionKey, state); } catch {}
          ...
          if (vague || cls.kind === 'PARTIAL' || cls.kind === 'HINT' || isNo || cls.reason === 'MAX_HINTS' || cls.reason === 'SEM_LOW') {
            ...
            if (attempts < 2 && (cls.kind === 'HINT' || vague || isNo)) {
              ...
              // HINT: objetivo-primero (sustituye el bloque actual de hint)
              {
                ...
                const deterministic = mkFb({ kind: cls.kind, matched: cls.matched, missing: cls.missing }, { attempts, hintsUsed, coursePolicies });
                ...
                state.lastFollowUpText = followUp;         // ← GUARDAR para reemisión
            ...
            {
              const momentCode = state.plan?.moments?.[act.step.momentIndex]?.code || '';
