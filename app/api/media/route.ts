...
type MediaMap = Record<string, { images: string[] }>;
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (step) {
      const m = media[String(step)] || { images: [] };
      return new Response(JSON.stringify({ step, images: Array.isArray(m.images) ? m.images : [] }), {
        status: 200,
